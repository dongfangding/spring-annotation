...
public class DevelopmentProfileCondition implements Condition {
    ...
    public boolean matches(ConditionContext context, AnnotatedTypeMetadata metadata) {
        ...
        // 如果是dev环境，并且当前IOC容器中未定义productionService则返回true，同时注册一个DevelopmentServiceLog
        if ("dev".equals(profile)) {
            if (!registry.containsBeanDefinition("productionService")) {
                RootBeanDefinition devServiceLogBean = new RootBeanDefinition(
                        "com.ddf.spring.annotation.bean.DevelopmentBeanLog");
                registry.registerBeanDefinition("developmentServiceLog", devServiceLogBean);
                return true;
            }
        }
        return false;
